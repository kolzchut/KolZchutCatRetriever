...
namespace KolZchutCatRetriever
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			var outputFilename = System.IO.Path.GetFileNameWithoutExtension(inputFilename) + "_RESULTS" + System.IO.Path.GetExtension(inputFilename);
			if (args.Count () > 1) {
			...
			while (!inputFile.EndOfStream)
			{
				...
				try
				{
					...
					if (categories == null || categories.Count == 0)
						throw new Exception( "No categoriess" );
				...
				{
					outputFile.WriteLine(line + @", ERROR!" + ex.Message );
				}
		...
		private static List<string> ExtractCategories(string pageText)
		{
			try
			{
				var catlinksMatch = Regex.Match(pageText, "<div id=\"mw-normal-catlinks\" class=\"mw-normal-catlinks\">(.|\n)*?</div>");
				var catlinks = catlinksMatch.Value.Trim();
				XmlDocument docCatLinks = new XmlDocument();
				docCatLinks.LoadXml(catlinks);
				//Console.WriteLine( String.Format("Links: {0}", catlinks.ToString() ) );
